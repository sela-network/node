...
import { copyToClipboard } from './node/text.utils';
...
updateElectronApp({
	updateSource: {
		...
		baseUrl: `https://storage.googleapis.com/sela_network-node-releases/releases/${process.platform}/${process.arch}`
	}
});
...
const createWindow = (): void => {
	...
	ipcMain.on('copy-to-clipboard', (_, text) => copyToClipboard(text));
	...
	session.defaultSession.webRequest.onHeadersReceived((details, callback) => {
		callback({
			responseHeaders: {
				...
				'Content-Security-Policy': ["default-src 'self' 'unsafe-eval' 'unsafe-inline' static: http: https: ws:; worker-src 'self' blob:;"]
			}
		})
	})
